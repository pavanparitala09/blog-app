    ...
    <div className="p-8 max-w-7xl mx-auto min-h-screen bg-slate-50">
      <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
        ...
        <button
          ...
          className="bg-violet-600 text-white px-8 py-3 rounded-full font-bold hover:bg-violet-700 transition-all shadow-lg hover:scale-105"
        >
          + New Article
        </button>
      </div>
      ...
      {articles.length === 0 && !loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {articles.map((article) => (
            <div
              key={article._id}
              className={`bg-white p-7 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-between transition-all group ${
                article.isArticleActive === false
                  ? "opacity-60 bg-slate-50 grayscale-[0.3]"
                  : "hover:shadow-xl"
              }`}
            >
              <div>
                <div className="flex justify-between">
                  <span className="px-3 py-1 bg-violet-50 text-violet-700 text-[10px] font-black rounded-full uppercase tracking-widest">
                    {article.category}
                  </span>
                  {article.isArticleActive === false && (
                ...
                <h3 className="text-2xl font-bold text-slate-800 mt-5 mb-3 group-hover:text-violet-600 transition-colors leading-tight">
                  {article.title}
                </h3>
              ...
              <div className="flex items-center justify-between pt-5 border-t border-slate-50">
                ...
                <button
                  ...
                  className={`${article.isArticleActive === false ? "bg-red-500" : "bg-slate-900"} text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-violet-600 transition-colors`}
                >
                  {article.isArticleActive === false
                    ? "View & Restore"
                    : "View Full"}
