  ...
  const linkStyles = ({ isActive }) =>
    `transition-all duration-300 px-4 py-2 rounded-lg ${
      isActive 
        ...
        : "text-white hover:bg-violet-300"
    }`;

  return (
    <nav className="bg-violet-500 shadow-lg sticky top-0 z-50">
        ...
        <div className="flex justify-between items-center h-24">
          ...
          <div className="flex-shrink-0 flex items-center gap-3">
            <img 
              ...
              alt="logo" 
            />
            <span className="text-white font-black text-3xl tracking-tight hidden sm:block">
              BlogApp
            </span>
          </div>
          ...
          <div className="hidden md:flex items-center gap-4">
            {!currentUser ? (
              ...
              <div className="flex items-center gap-4 border-l border-violet-400 pl-4">
          ...
          <div className="md:hidden flex items-center">
            <button 
              onClick={toggleMenu}
              className="text-white focus:outline-none p-2 rounded-md hover:bg-violet-400 transition-colors"
            >
      ...
      <div className={`md:hidden transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <ul className="px-4 pt-2 pb-6 space-y-2 bg-violet-600 shadow-inner">
          {!currentUser ? (
            <>
              <li><NavLink onClick={() => setIsOpen(false)} className="block py-3 px-4 text-white font-bold rounded-lg hover:bg-violet-500" to="/">Home</NavLink></li>
              <li><NavLink onClick={() => setIsOpen(false)} className="block py-3 px-4 text-white font-bold rounded-lg hover:bg-violet-500" to="/register">Register</NavLink></li>
              <li><NavLink onClick={() => setIsOpen(false)} className="block py-3 px-4 text-white font-bold rounded-lg hover:bg-violet-500" to="/login">Login</NavLink></li>
            </>
          ) : (
            <div className="pt-4 border-t border-violet-400 mt-2">
